...
$map_width  = (int) $field->parameters->get('map_width', 350);
$map_height = (int) $field->parameters->get('map_height', 250);
 ...
 * TODO replace this and instead use single method that reads data properties from every map's container DIV
 */
...
foreach ($this->values as $n => $value)
{
	...
	if ($show_map && strlen($value['lon']) && strlen($value['lat']))
	{
		if ($map_api === 'googlemap')
		{
            ...
            data-mapicon="'.$value["custom_marker"].'"
						id="' . $map_tagid . '"' .
...
if ($addressint_view_js_added === null && $map_embed_type === 'int' && $map_api === 'googlemap')
{
    ...
    var mapIcon = mapBox.attr("data-mapicon");
